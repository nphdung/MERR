...
demands = l_demand
#demands = [(19, 36), (10, 14), (48, 14), (38, 31), (36, 38), (48, 33), (27, 5), (40, 39), (23, 13), (9, 37)]
...
l_demands = list()  # list of corresponding length
for (s,t) in demands:
    d = (s,t)
    print(f"\nDemand {d}: ",end="")
    temp = s
    ...
    m_demands.append(d)
    l_demands.append(len(path)-1)

fptr.write(f"Satisfied demands: {m_demands}\n")
fptr.close()
sptr = open("Sample_minmax.txt",'a')
for (u,v) in m_demands:
    sptr.write(f"({u},{v}) ")
sptr.write("\n")
sptr.close()
lptr = open("length.txt",'a')
lptr.write(f"{max(l_demands)}")
lptr.write("\n")
lptr.close()