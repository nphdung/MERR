...
for d in demands:
    prob += pulp.lpSum([lp_vars[d,e] for e in edges if (e[0] == d[0])]) - \
            pulp.lpSum([lp_vars[d,e] for e in edges if (e[1] == d[0])]) <= 1
...
while True:
    ...
    for (u,v) in u_edges:
        temp = 0
        for d in demands:
            temp = temp + flow[d,(u,v)] + flow[d,(v,u)]
